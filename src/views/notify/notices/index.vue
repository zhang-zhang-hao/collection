<template>
    <div class="app-container">
        ...
        <el-table v-loading="loading" :data="bannerList" fit>
            ...
            <el-table-column label="链接" prop="gotoUrl" />
            ...
            <el-table-column label="操作" align="center" fixed="right" width="200">
                <template #default="scope">
                    ...
                    <el-button type="text" icon="Delete" @click="delNotice(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog :title="type === 'add' ? '新增': '编辑'" v-model="open" width="800px" append-to-body>
            <el-form ref="roleRef" :model="form" :rules="rules" label-width="100px" label-position="left">
                <el-form-item label="类型" prop="title">
                    <el-input v-model="form.title" type="text" size="size" auto-complete="off" placeholder="请输入标题">
                    </el-input>
                </el-form-item>
                <el-form-item label="富文本" prop="content" style="margin-bottom: 70px;">
                    <!-- <quill-editor :disabled="true" theme="snow" content-type="html" enable v-model="form.content" />
                     -->
                    <QuillEditor theme="snow" v-model:content="form.content" contentType="html" ref="quill" />
                </el-form-item>
                <el-form-item label="链接" prop="gotoUrl">
                    <el-input v-model="form.gotoUrl" type="text" size="size" auto-complete="off">
                    </el-input>
                </el-form-item>
...
<script setup >
import { getNoticesList, delNotices, addNotices } from '@/api/banner'
...
const submitForm = async () => {
    let data = form.value
    const res = await addNotices(data)
    console.log(res)
    if (res.code === 200) {
        ElMessage({
            message: '新增成功',
...
const getNotices = async () => {
    ...
    if (res.code === 200) {
        bannerList.value = res.data.notices
    }
}
...
const handleAdd = () => {
    ...
    type.value = 'add'
    nextTick(() => {
        quill.value.setHTML(form.value.content)
    })
}
...
const delNotice = async (id) => {
    ...
    const res = await delNotices(data)
    if (res.code === 200) {
        ElMessage({
            message: '操作成功',
            type: 'success',
