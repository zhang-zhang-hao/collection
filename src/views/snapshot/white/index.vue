<template>
    <div class="app-container">
        <!-- 表格数据 -->
        <el-table v-loading="loading" :data="whiteList" fit>
...
<script setup >
import { getWhiteList } from '@/api/snapshot'
...
const getWhite = async () => {
    const res = await getWhiteList(queryParams.value)
    if (res.code === 200) {
        whiteList.value = res.data.records
    }
}
getWhite()
